...
// yxc的二分模板
// 找出第一个大于等于target的数
bool check1(int i, int target) { return i >= target; }
int bs1(const std::vector<int> &vi, int target, int l, int r) {
    while (l < r) {
        int mid = (l + r) / 2;
        if (check1(vi[mid],
                   target)) {  // 满足条件，当然是 xx = mid，因为mid不能排除
                               // 找第一个因此右边界收缩
            r = mid;
        } else {
            l = mid + 1;
...
// 找出最后一个小于等于target的数
bool check2(int i, int target) { return i <= target; }
int bs2(const std::vector<int> &vi, int target, int l, int r) {
    while (l < r) {
        int mid =
            (l + r + 1) /
            2;  // l = mid需要向上取整，防止死循环  找最后一个，所以左边界收缩
