...
// 执行用时分布16ms击败21.49%
// 消耗内存分布17.34MB击败14.91%
class Solution260615 {
public:
  string convert(string s, int numRows) {
    ...
    while (i < sz) {
      ...
      for (int j = 0; j < numRows - 2 && i < sz; j++, i++) {
        matrix[numRows - 2 - j].push_back(s[i]);
      }
    }
    string res;
    for (int i = 0; i < numRows; i++) {
      for (const char &ch : matrix[i]) {
        res.push_back(ch);
      }
    }
    return res;
...
string secondconvert(string s, int numRows) {
  ...
  for (int base = 0, gapbase = gapsum; base < numRows && base < n;
       base++, gapbase -= 2) {
